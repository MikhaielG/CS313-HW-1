...
int main() {
	...
	*/
	...
	for (int i = 0; i < size + 1; i++) {
		char temp = char(33 + rand() % 95);	//for getting between char(33) and char(127)
		string x;
		x.push_back(temp);
	...
	//populating the list
	for (int i = 0; i < size; i++) {
		char x = char((int)33 + rand() % (int)95);
		string y;
		y.push_back(x);
	...
	//1c
	vector<string> cVector;	//vector populated with move semantics
	//populating starts...
	auto movesem_vec_begins = high_resolution_clock::now();

	for (int i = 0; i < size + 1; i++) {
		char temp = char(33 + rand() % 95);	//for getting between char(33) and char(127)
		string x;
		x.push_back(move(temp));
		cVector.push_back(move(x));
	}
	//populating ends...
	auto movesem_vec_ends = high_resolution_clock::now();

	auto time_for_movesem_vec = duration_cast<microseconds>(movesem_vec_ends - movesem_vec_begins);
	cout << "\nElapsed time for populating the vector with random strings using Move semantics: " << time_for_movesem_vec.count() << " micro sceconds!" << endl;
	
	//printing the vector's 1st 10 elements to check populating
	/*for (int i = 0; i < 10; i++) {
		cout << cVector[i] << endl;
	}*/
	// time taken
	...
	list<string> cList;
	// time when the string list starts populating
	auto movesem_List_begins = high_resolution_clock::now();
	//populating the list
	for (int i = 0; i < size; i++) {
		char x = char((int)33 + rand() % (int)95);
		string y;
		y.push_back(move(x));
		cList.push_back(move(y));
	}
	// time when the string list finishes populating
	auto movesem_List_ends = high_resolution_clock::now();

	// printing the list
	/*int index = 1;
	list<string> ::iterator ita;
	for (ita = cList.begin();ita != cList.end(); ita++) {
		cout << index << "->\t" << *ita << endl;
		index += 1;
	}*/

	// time taken
	auto time_for_movesem_list = duration_cast<microseconds>(movesem_List_ends - movesem_List_begins);

	cout << "Elapsed time for populating the list with random strings using Move semantics: " << time_for_movesem_list.count() << " micro sceconds!" << endl;

	cout << "\nAns for 1C ->";

	if (time_for_movesem_list < time_for_movesem_vec)
		cout << "\nUsing list is better for random strings when using move semantics!" << endl;
	else
		cout << "\nUsing vector is better for random strings when using move semantics!" << endl;
}
